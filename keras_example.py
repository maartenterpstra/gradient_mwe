...
class UnrolledNetwork(tf.keras.Model):
    def __init__(self):
        super().__init__()
        self.layer = ComplexConv2D(filters=1, kernel_size=(3,3), padding="SAME", kernel_initializer=my_init, bias_initializer=tf.keras.initializers.Zeros())
...
def complex_conv2d(x, weight, padding="VALID", strides=1, dilations=1):
    ...
    return complex_conv(functools.partial(
        nn_ops.convolution_v2,
        strides=[1,1],
        padding='SAME',
        dilations=[1,1],
        data_format='NCHW',
        name='conv'), x, weight, padding=padding, strides=strides, dilations=dilations)
...
if __name__ == "__main__":
    ...
    inputs = [tf.convert_to_tensor(adjoint)]
